...
def nfl_article_image_processing():
    ...
    def get_new_images():
        ...
        s = utils.read_sql(SQL_DIR / "unprocessed-article-images.sql")
        print(s)
        imgs = utils.get_df(s)
        print(f"articles shape: {imgs.shape}")
        # Convert datetime columns to strings for XCom JSON serialization
        datetime_cols = imgs.select_dtypes(include=["datetime64[ns]", "datetime64[ns, UTC]"]).columns
        for col in datetime_cols:
            imgs[col] = imgs[col].dt.strftime("%Y-%m-%dT%H:%M:%SZ")
